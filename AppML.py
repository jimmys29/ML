            ...
            while PAGINACION_MAX > PAGINACION_ACTUAL:
                ...
                for link in links_de_la_pagina:
                    ...
                    try:
                        ...
                        precio_texto = driver.find_element ( By.XPATH ,
                                                             '//span[contains(@class,"andes-money-amount ui-pdp-price__part andes-money-amount--cents-superscript andes-money-amount--compact")]' ).text
                        precio_numero = re.search ( r'\d+' , precio_texto ).group ()
                        descripcion = driver.find_element ( By.XPATH ,
                                                            '//p[@class="ui-pdp-description__content"]' ).text
                        ...
                        valuacion = driver.find_element ( By.XPATH ,
                                                          "//*[@id='reviews_capability_v3']/div/section/div/div[1]/article/div/div[1]/div[1]/p" ).text
                        vendedor = driver.find_element ( By.XPATH ,
                                                         "//p[@class='ui-seller-info__status-info__title ui-pdp-seller__status-title']" ).text
                        ver_mas_caracteristicas = driver.find_element ( By.XPATH ,
                                                                        '//span[@class="ui-pdp-collapsable__action" and text()="Ver más características"]' )
...
if os.path.exists ( 'productos_streamlit.csv' ):
    ...
    if len ( df ) > 0:
        ...
        price_range = st.slider ( "Filtrar por rango de precio" , float ( new_df [ "Precio" ].min () ) ,
                                  float ( new_df [ "Precio" ].max () ) ,
                                  (float ( new_df [ "Precio" ].min () ) , float ( new_df [ "Precio" ].max () )) ,
                                  step=10.0 )
